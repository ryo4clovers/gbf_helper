...
**未解決の課題(規模の問題)**: このジョブLv詳細レスポンスがどの画面操作から得られるか(具体的な遷移手順)はまだ確認できていない。また、上述の「実機APIへの直接fetch()は禁止」ルールにより、Claude側から一括取得することはできないため、残り76ジョブ分をこの構造に合わせて作り直すには、**ユーザーに1ジョブずつ同様のデータを取得・共有してもらう必要がある**。これは前回の一括収集(fetch()ループ、現在は禁止)よりもユーザーの作業負荷が大きいため、全77ジョブを同じ密度で揃えるかどうかは要相談。当面は、新しくデータが提供されたジョブから順にこの構造へ更新していく方針とする。
