...
**残作業/次にやること**: 77ジョブ分の`master`データは`all_job`から一括取得済み(scratchpadの`job_api_master_data.tsv`、リポジトリ未コミットの作業ファイル)。`job_equipped`は1ジョブ(マナダイバー)のみ詳細を確認済みで、残り76ジョブ分の詳細(アビリティ本文・サポート・マスターレベル強化)はまだ取得していない。全80ジョブ(オリジン3件はこのアカウントでは`all_job`から直接は取れておらず、`class:1`内の進化済み表示から間接的に確認)の本文収集を始める場合、この`job_equipped`をジョブごとに叩くループを組めば良い。
