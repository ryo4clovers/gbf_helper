...
- 水属性R: scratch-build-list.mjsでリスト取得10体。GameWith全10体のページ取得完了、gbf.wiki検索は9/10体で候補あり(curl経由で取得)。dossier生成完了(batch_000-007.txt〜batch_008-009.txt)。次はindex0から処理開始。index0 猫は「主人公と同属性」の特殊仕様キャラでGameWithが火属性ページとして分類。gbf.wiki候補「Karyl」は無関係の別キャラと判断し不採用、GameWithのみで作成。  - index119 ネツァワルピリはgbf.wiki候補なし。GameWithのみで作成。最終上限解放後の性能を採用。
