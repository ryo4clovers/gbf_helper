...
- 出典: ユーザーが通常のゲーム内操作で取得した永遠拒絶の大鎌Lv250・SLv25・超越phase5の武器詳細。初回共有の絶対否定の大鎌とその編成・木人ログは通常版だったため、方陣版の検証根拠から除外した。
- 武器ID`1040310700`、ATK4440・HP318（+0）、`skill_id: 2198`「機炎方陣・神醒III」、`2209`「ベータ・リベレイション・アルブスII」、`1726`「虚偽と詐術」を実機確認した。
- 神醒IIIを方陣攻刃24%・最大HP24%・与ダメージUP2%の3効果へ分解し、方陣加護の対象として暫定接続した。Lv230時点の与ダメージUP1.75%に対し、Lv250では2%へ強化される整理とした。
- 与ダメージUPは、攻撃・防御・減衰／上限処理後に乗算する後段効果としてモデル化した。通常攻撃・追撃・多段アビリティ・奥義の各ヒットに適用されるが、今回は通常攻撃計算だけへ接続した。ローカル計算機の「その他スキル」へ加護適用後の値を表示する。
- コロッサス・マグナ170%時の暫定計算値は、方陣攻刃64.8%・HP64.8%・与ダメージUP5.4%。方陣版を編成したレスポンスは未取得のため、これらは下書き・要検証を維持する。
- 生レスポンスは`tools/network-recorder/captures/weapons/2026-09-14_weapon-dark-opus-scythe-magna-lv250-slv25.json`へローカル保存し、コミット・公開対象外とする。
- 保留: 方陣版の編成表示、コロッサス・マグナ加護後の3効果、Lv230〜249の効果量、減衰・上限付近、追撃・アビリティ・奥義のヒット単位照合、第2・第3スキルの数値。
