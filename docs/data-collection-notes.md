...
ただし `サポートアグニス_+7.json` は内部値が+6（武器ATK 2,200、主人公攻撃力22,847）で、
+6ファイルと同一内容だったため重複点として除外した。有効な+0〜+6・+8〜+10の10点は次の通り。
...
差は一定ではなく、通常側は0〜+5、対有利側は+1〜+7へ拡大した。最終切り捨てだけでは説明できず、
...
- 生データ保存先: `tools/network-recorder/captures/calculator/2026-09-06_deck_support-agni_plus01.json`〜
  `plus10.json`（gitignore対象。plus07は内部的に+6の重複データ）
