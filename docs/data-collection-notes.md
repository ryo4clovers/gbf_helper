...
- **進捗(火属性SSR)**: list.jsonのインデックス0〜20まで処理済み(21/111体)。既存の`fire-ssr-percival-normal.md`はindex102「パーシヴァル」(無印)と重複するため、その番になったらスキップする。
  ...
  - **進捗**: index87まで処理済み(88/111体、`batch_080-087.txt`完了)。続きを行う場合: 次バッチ(index88〜)のダウジエをまだ生成していないため、`scratch-fetch.mjs`/`scratch-pick-and-dump.mjs`/`scratch-build-dossier.mjs`で次バッチを生成してから読み進める。
