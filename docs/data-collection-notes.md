...
- 闇属性SR(5/35体完了)。index1 メグはgbf.wiki候補「Meg and Mari」がまり(Mari)とのペアキャラ別ページと判断し不採用、GameWithのみで作成。index2 ヨウはgbf.wiki候補「You」が光属性・SSR相当ステータスの別バージョンと判断し不採用、GameWithのみで作成。次はindex5(カシウス)から継続(batch_000-007.txt)。
