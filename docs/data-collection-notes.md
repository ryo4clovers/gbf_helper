...
- 闇属性SR(7/35体完了)。index1 メグはgbf.wiki候補「Meg and Mari」がまり(Mari)とのペアキャラ別ページと判断し不採用、GameWithのみで作成。index2 ヨウはgbf.wiki候補「You」が光属性・SSR相当ステータスの別バージョンと判断し不採用、GameWithのみで作成。- 闇属性SR(24/35体完了)。index19 神崎蘭子は変身システム(『闇の力』10で目覚めし鼓動発動)を持つ特殊仕様のキャラ。gbf.wiki候補なし。GameWithのみで作成、目覚めし鼓動発動後の性能を中心に簡略化して記載。次はindex24(白坂小梅)から継続(batch_024-031.txt)。
