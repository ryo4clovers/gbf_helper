...
- 火の渾身（`skill_id: 1296`）SLv15は、HP50%で通常スキル強化340%時の実機表示が11.3%だった。現行式の基礎2.57%へ`×4.4`を適用した11.308%と一致する。
...
- 通常渾身はHP25%未満を0%、通常背水は`基礎値 × (1 + 2 × HP減少率) × HP減少率`として計算。渾身は式の基礎効果量を小数第2位へ丸めてから加護を適用する。
