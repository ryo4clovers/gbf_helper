...
- 風属性SR(10/49体完了)。index1 プロトジョヤ(ジョイ)はgbf.wiki候補「Proto-Joya」が同名召喚石ページ(キャラは別名"Joy (Event)")と判断し不採用、GameWithのみで作成。index2 クルーニ(SR)はgbf.wiki候補「Krugne」がHP720/ATK3600の基本レアリティ(R)版と判断し不採用(GameWith記載SR版はHP750/ATK6500)、GameWithのみで作成。index3 制服コルワはgbf.wiki候補「Side-scrolling Quotes」が無関係のページと判断し不採用、GameWithのみで作成。次はindex10(アーサー)から継続(batch_008-015.txt)。gbf.wiki候補「Arthur and Mordred」は既にindex5で使用済みの別キャラページのため不採用予定(アーサー単独版の候補は自動検索で見つからず)。
