...
- **進捗**: list.jsonのインデックス0〜35まで処理済み(index 0の水着ハイラは既存の`payila-summer-ssr.md`と重複のためスキップ、実質35体作成)。index 36以降が未処理。gbf.wiki候補が見つからず/信頼できずGameWithのみで作成した5体(光ミニゴブ、光ナーヴェ、サンダルフォン(リミテッド)、ロベルティナ(光)、ボレミア(光))あり。
