...
- **進捗**: list.jsonのインデックス0〜28まで処理済み(index 0の水着ハイラは既存の`payila-summer-ssr.md`と重複のためスキップ、実質28体作成)。index 29以降が未処理。gbf.wiki候補が見つからず/信頼できずGameWithのみで作成した3体(光ミニゴブ、光ナーヴェ、サンダルフォン(リミテッド))あり。
