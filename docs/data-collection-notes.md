...
- 出典: ユーザーが通常の編成操作で取得したゲーム内`deck`レスポンス2件。ウィルナスあり/なし以外は同一で、主人公ATK 22801・HP 4877、召喚石ATK 4157・HP 1414も一致した。生データは個体ID等を含むため`tools/network-recorder/captures/summon-sub-aura/`へローカル保存し、コミット対象外とした。
...
- 保留: 3凸時点のサブ加護倍率、通常召喚石枠とサブ加護枠の双方での発動差、同系統サブ加護の複数加算。
...
- 保留: 3凸時点の各倍率、通常召喚石枠とサブ加護枠の発動差、同系統サブ加護の複数加算、他5属性の編成表示値差分。
