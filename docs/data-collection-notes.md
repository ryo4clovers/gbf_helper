...
- 恩寵の最大HP（小）は既存の守護量表からSLv15で12%として下書き接続した。治癒の他SLvも二次資料由来の下書きとし、実機確認済みのSLv15効果とは効果単位で確度を分離した。
...
- 保留: 治癒III・恩寵のSLv1〜14、恩寵の最大HP量の単独実機検証、弱体耐性の詳細な命中判定・上限、ポーション以外の各種回復効果との実戦照合。
