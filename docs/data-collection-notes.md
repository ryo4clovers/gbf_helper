...
## 大事なもの等のDA/TA補正を暫定反映(2026-09-07)

- ユーザーが、連撃スキル武器を装備していない現在のナイト編成でDA率14%・TA率8%と表示されることを実機画面で確認した。ナイトの登録済み基礎率DA 7%・TA 3%との差はDA +7%・TA +5%。
- 既存収集記録では「赫焔灯す菩薩の印章」に火属性キャラのDA・TA強化が含まれることまで確認済みであり、今回の差分は大事なもの効果による可能性が高い。ただし個別アイテムの効果量と他の恒常補正の有無をまだ切り分けていないため、帰属は仮説とする。
- ローカル計算画面の環境倍率へ「大事なもの等 DA」「大事なもの等 TA」を追加し、現在の検証環境に合わせて7%・5%を暫定初期値とした。値は他アカウントや取得状況に合わせて変更でき、戦闘画面ではジョブ・育成・武器寄与と合算した14%・8%を表示する。
- `effective_acquired_bonus`に「ダブルアタック確率」「トリプルアタック確率」またはDA/TA率が含まれる生データを今後取り込んだ場合、自動的にアカウント補正へ正規化できる解析経路も追加した。
- 要検証: DA +7%・TA +5%が赫焔灯す菩薩の印章単独の値か、複数の大事なもの・アカウント効果の合計かを、取得状況または実機レスポンスで確認する。
