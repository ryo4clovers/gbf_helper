...
- 風属性SR(18/49体完了)。index1 プロトジョヤ(ジョイ)はgbf.wiki候補「Proto-Joya」が同名召喚石ページ(キャラは別名"Joy (Event)")と判断し不採用、GameWithのみで作成。index2 クルーニ(SR)はgbf.wiki候補「Krugne」がHP720/ATK3600の基本レアリティ(R)版と判断し不採用(GameWith記載SR版はHP750/ATK6500)、GameWithのみで作成。index3 制服コルワはgbf.wiki候補「Side-scrolling Quotes」が無関係のページと判断し不採用、GameWithのみで作成。index10 アーサー(SR)はgbf.wiki候補「Arthur and Mordred」がHP1100/ATK6300のペアキャラ別ページ(GameWith記載の単独版はHP980/ATK5100)と判断し不採用、GameWithのみで作成。index12 カルテイラ(SR)はgbf.wiki候補「Karteira」がHP940/ATK3740の基本レアリティ(R)版と判断し不採用(GameWith記載SR版はHP1390/ATK4650)、GameWithのみで作成。index13 水着クロエはgbf.wiki候補「Chloe (Summer)」がタイトル/声優/ATKは一致するがHPに差異あり(GameWith900/gbf.wiki500)、GameWith主体・両出典併記で作成。次はindex18(アリサ(シャドバ))から継続(batch_016-023.txt)。
