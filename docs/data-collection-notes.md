...
- 出典: ユーザーが通常の編成操作で取得したゲーム内`deck`レスポンス3件。ウィルナスなし、サブ加護召喚石枠、サブ召喚石枠を比較した。生データは個体ID等を含むため`tools/network-recorder/captures/summon-sub-aura/`へローカル保存し、コミット対象外とした。
...
- 保留: 3凸時点の各倍率、同系統サブ加護の複数加算、他5属性の編成表示値差分。
