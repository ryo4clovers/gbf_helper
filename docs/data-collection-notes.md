...
- **進捗**: list.jsonのインデックス0〜23まで処理済み(index 0の水着ハイラは既存の`payila-summer-ssr.md`と重複のためスキップ、実質23体作成)。index 24以降が未処理。gbf.wiki候補が見つからずGameWithのみで作成した2体(光ミニゴブ、光ナーヴェ)あり。
