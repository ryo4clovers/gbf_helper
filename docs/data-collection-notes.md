...
- 水属性R(1/10体完了)。index0 ハイラ(R)は攻撃行動をしない特殊なR十二神将。gbf.wiki候補なし。GameWithのみで作成。次はindex1(ランドル(R))から継続(batch_000-007.txt)。index0 猫は「主人公と同属性」の特殊仕様キャラでGameWithが火属性ページとして分類。gbf.wiki候補「Karyl」は無関係の別キャラと判断し不採用、GameWithのみで作成。  - index119 ネツァワルピリはgbf.wiki候補なし。GameWithのみで作成。最終上限解放後の性能を採用。
