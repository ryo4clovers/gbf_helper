...
## グレネードで通常攻刃（小）SLv1を検証(2026-09-13)

- 出典: ユーザーが通常の編成操作で取得したゲーム内武器詳細レスポンスと編成レスポンス。グレネード（武器マスターID `1020500200`、Lv1、SLv1、0凸、HP11、ATK215）と、火の攻刃（skill id `1`）を確認した。
- 編成はスキルなしのブロンズソードとグレネードのみで、通常武器スキル強化は170%、攻刃表示は2.7%だった。`1% × (1 + 1.70) = 2.7%`と一致するため、火の攻刃（小）SLv1の基礎効果量1%を実機確認済みとした。
- `knowledge/mechanics/weapon-normal-attack-skills.md`を追加し、計算用スキルカタログへSLv1の効果を登録した。グレネードの計算用武器カタログは火の攻刃（skill id `1`）へ接続した。
- 武器カタログのLv40ステータスは公開Wiki由来で実機未確認のため、武器全体のステータスは下書きを維持した。
- 生データは`tools/network-recorder/captures/weapons/2026-09-13_deck_grenade-fire-might-small-slv1.json`と`2026-09-13_weapon_grenade-fire-might-small-slv1.json`へローカル保存し、コミット対象外とした。
- 保留: 通常攻刃（小）のSLv2〜14・16〜20、他属性のskill_id、通常攻刃（中・大・II・III等）のSLv別効果量。
