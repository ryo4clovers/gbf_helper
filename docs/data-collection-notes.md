...
- 出典: ユーザーが通常の編成操作で取得したゲーム内`deck`レスポンス4件。ウィルナスなし、サブ加護召喚石枠、サブ召喚石枠、4凸+無凸の2枚配置を比較した。生データは個体ID等を含むため`tools/network-recorder/captures/summon-sub-aura/`へローカル保存し、コミット対象外とした。
