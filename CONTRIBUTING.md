...
- ローカル計算機の主人公を右クリックするか「強化設定」を押すと、ジョブLv・ML・極致・攻撃力LB・HP LB・現在の主人公属性に対応する属性攻撃LB（無印・II・III）を入力できます。非対応のML・極致は無効表示です。
- LBは★0〜★3で、攻撃力は累計0/500/1500/3000、HPは0/300/600/1000。全体コンプリート補正前の小計に加算します。6属性の属性攻撃LBは各3項目あり、それぞれ累計0/1/3/5%を一致する属性の属性攻撃枠へ加算します。火属性は実戦検証済み、他5属性の計算モデルは下書きです。端数処理は表示ステータスと通常攻撃の既存モデルに従います。
