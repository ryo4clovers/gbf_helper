...
- ローカル計算機の主人公を右クリックするか「強化設定」を押すと、ジョブLv・ML・極致と、ダメージ計算やバトルに影響する主人公LBを入力できます。計算接続済み39項目は従来の個別フィールド、未接続項目はゲーム内IDをキーとする`otherLimitBonusLevels`へ保存します。獲得EXP・RANKポイント系の2項目は画面に出しませんが、旧保存データとの互換性は維持します。LBは用途別のアイコン付きグループへ分け、各見出しに設定数と「計算接続済み」「一部未接続」「入力・保存のみ」のいずれかを表示します。非対応のML・極致は無効表示です。
