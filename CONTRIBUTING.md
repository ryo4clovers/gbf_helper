...
- ローカル計算機の主人公を右クリックするか「強化設定」を押すと、ジョブLv・ML・極致・攻撃力LB・HP LB・得意武器攻撃LB全8項目・現在の主人公属性に対応する属性攻撃LB（無印・II・III）を入力できます。非対応のML・極致は無効表示です。
- LBは★0〜★3で、攻撃力は累計0/500/1500/3000、HPは0/300/600/1000。全体コンプリート補正前の小計に加算します。得意武器攻撃LBは各0/1/3/5%で、第1専用3項目、第2専用3項目、両方が対象の2項目を分け、同じ武器に有効な率を合算して武器単位で四捨五入します。6属性の属性攻撃LBは各3項目あり、それぞれ累計0/1/3/5%を一致する属性の属性攻撃枠へ加算します。第2専用LBの正方向と、火以外の属性攻撃LBの計算モデルは下書きです。
