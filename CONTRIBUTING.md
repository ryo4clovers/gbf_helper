...
- ローカル計算機の主人公を右クリックするか「強化設定」を押すと、ジョブLv・ML・極致と、実機カタログで確認した主人公LB全88項目を入力できます。計算接続済み39項目は従来の個別フィールド、未接続49項目はゲーム内IDをキーとする`otherLimitBonusLevels`へ保存します。LBは用途別のアイコン付きグループへ分け、各見出しに設定数と「計算接続済み」または「入力・保存のみ」を表示します。非対応のML・極致は無効表示です。
