...
- ローカル計算機の主人公を右クリックするか「強化設定」を押すと、ジョブLv・ML・極致・攻撃力LB・HP LB・味方全体HP LB全3項目・得意武器攻撃LB全8項目・連撃系LB全5項目・現在の主人公属性に対応する属性攻撃LB（無印・II・III）を入力できます。非対応のML・極致は無効表示です。
- LBは★0〜★3で、攻撃力は累計0/500/1500/3000、HPと味方全体HPの各項目は0/300/600/1000。全体コンプリート補正前の小計に加算します。味方全体HP LBは現時点で主人公HPだけに接続しています。得意武器攻撃LBは各0/1/3/5%で、第1専用3項目、第2専用3項目、両方が対象の2項目を分け、同じ武器に有効な率を合算して武器単位で四捨五入します。連撃系LBはDA3項目・TA2項目を各0/1/3/5%として既存の非武器連撃率へ加算します。6属性の属性攻撃LBは各3項目あり、それぞれ累計0/1/3/5%を一致する属性の属性攻撃枠へ加算します。キャラクターへの味方全体HP LB、連撃系LBの実戦適用、第2専用LBの正方向、火以外の属性攻撃LBの計算モデルは下書きです。
- LB選択は編成に保存します。ジョブ変更時は★0へ戻します。実機deckには振り分けがないため、変換後は現在のLBを手動設定してください。
