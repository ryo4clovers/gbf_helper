...
- LBは★0〜★3で、攻撃力は累計0/500/1500/3000、HPと味方全体HPの各項目は0/300/600/1000。全体コンプリート補正前の小計に加算します。クリティカルLBは無印・II・IIIを各0/1/3/5%の独立抽選として扱い、発動時に同率のダメージを加算します。武器クリティカルとは別に抽選し、同時発動時は武器側のダメージボーナスへ加算します。通常技巧率は発動率100%を上限とし、超過分の半分をオーバースキルのクリティカルダメージ表示へ変換し、その表示率で武器クリティカルの基本+50%を強化します。味方全体HP LBは現時点で主人公HPだけに接続しています。得意武器攻撃LBは各0/1/3/5%で、第1専用3項目、第2専用3項目、両方が対象の2項目を分け、同じ武器に有効な率を合算して武器単位で四捨五入します。連撃系LBはDA3項目・TA2項目を各0/1/3/5%として既存の非武器連撃率へ加算します。6属性の属性攻撃LBは各3項目あり、それぞれ累計0/1/3/5%を一致する属性の属性攻撃枠へ加算します。クリティカルLBは無印★1・★3、II★3、III★3、複数LBの同時発動、および武器技巧47.7%・82.15%・100%とIII★3の同時発動時の加算が実戦検証済みです。技巧47.7%ログでは両方不発・LBのみ・武器のみ・同時発動の4状態も確認済みです。無印★2、II・IIIの★1/2、オーバースキル変換率の別超過量は下書きです。キャラクターへの味方全体HP LB、連撃系LBの実戦適用、第2専用LBの正方向、火以外の属性攻撃LBの計算モデルも下書きです。
