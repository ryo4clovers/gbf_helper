...
- 現在の編成、サポート召喚石、敵条件、環境倍率、乱数設定をブラウザの端末内ストレージへ自動保存し、F5更新や戦闘画面から戻った際に復元する
- 計算条件一式を複数の名前付き編成として端末内へ保存し、一覧から読み込み・上書き・名前変更・削除できる
- 「端末に保存」で即時保存し、「JSON保存」で計算条件一式をバックアップする（従来の編成JSONも引き続き読込可能）
- 本家`deck.json`相当レスポンスを、個人・装備インスタンスIDを保持しない設定へ変換する
- メイン1枠と通常9枠の選択式エディタで、登録済み武器の検索・配置・削除・上限解放・Lv・SLv変更を行う
- Wiki・実機由来のレベル境界値がある武器は、任意の整数Lvから区間式でHP・ATKを自動再計算する。上限解放段階を下げた場合はLvをその段階の上限へ調整する
- メイン1枠・通常4枠・サブ加護2枠の選択式エディタで、登録済み召喚石の検索・配置・削除・Lv変更を行う
- 戦闘条件としてサポート召喚石を選択し、通常加護だけを反映する（HP/ATK、サブ加護、「メイン召喚石に装備時」の効果は対象外）
- 前衛3枠・サブ2枠のキャラクターをナレッジから選択し、実機表示HP/ATKを記録する（キャラクターのダメージ計算は未対応）
...
- 主人公ジョブ欄の属性チップで、現在の主人公属性を常時確認する
- 武器・召喚石へ0〜99のプラスボーナスを設定し、個別ATKへ1ポイント当たり5、HPへ1を反映する
- 敵属性、防御値、船、炉、大事なもの相当倍率、ジョブ通常攻撃与ダメージを入力する
- 通常攻撃本体、追撃、合計の最小・最大・期待値と計算段階を確認する
- ヘッダーの「戦闘シミュレート」から、現在の編成・敵・環境倍率を引き継いで検証画面を開く

戦闘シミュレート画面はターン1から開始し、通常攻撃の本体と追撃を記事モデルの独立乱数で
敵HPへ反映します。通常抽選に加え、最低乱数かつ100%未満のクリティカル・連続攻撃を除外する
「下振れ」と、最高乱数かつ発生率が正の効果を確定させる「上振れ」を切り替えられます。
DA/TA率は選択中ジョブの基礎率、ジョブLv・マスターレベル・極致の証の取得済みボーナス、
取得済みジョブ全体のコンプリートボーナス合計、登録済み武器スキルから自動算出します。
実機編成データの`job.bonue.master_bonus`に含まれるDA/TA合計を変換時に保持します。既定の検証編成では
確認済みのDA +7%・TA +5%を設定しています。神滅の印章によるDA/TAは本家予測表示へ加算しません。元に戻す、リセット、
味方HP・奥義ゲージ、アビリティ・召喚石・回復アイテム、行動時系列のUIも備えています。
通常武器スキルによるDA/TAはそれぞれ75%を上限とし、その後にジョブ等の別枠を加算します。
最終DA/TAは全寄与の合算後に小数点以下を切り捨てます。通常モードではTAを先に抽選し、失敗時のみ
DAを抽選するため、入力率を`t`・`d`とした実効確率はTA=`t`、DA=`(1-t)d`、SA=`(1-t)(1-d)`です。
ユーズド・木人は生存中、主人公の通常攻撃後に基礎攻撃力2,000の通常攻撃を行います。既存の
被ダメージ計算を利用し、通常モードは予測範囲内の乱数、下振れは最大値、上振れは最小値を採用します。
オールド・木人は攻撃しません。現時点で実計算に接続済みなのは主人公の通常攻撃、武器クリティカル、
追撃、ユーズド・木人の通常攻撃です。
奥義・アビリティ・召喚の効果量、敵の特殊行動、キャラクター効果・戦闘中バフによるDA/TA変化、
回復量の正式仕様は未実装または暫定です。
...
未設定のままにして `issues` へ警告と設定上のパスを返します。検証済みのレベル境界値がある武器は
Lvからステータスを導出し、それ以外でステータス上書きがない場合は警告します。

武器・武器スキルの増分カタログは `catalog/weapons.v1.json` と
`catalog/weapon-skills.v1.json` に分離しています。禁栄の禍槍にはLv1・100・150の境界ステータスを登録し、
Lv2〜100とLv101〜150の区間計算を実機12レベル分で確認しています。コロッサスケーン・マグナは
ゲーム内図鑑詳細のLv1値と実測Lv100・150・200の境界を登録し、Lv1〜200を区間計算します。
レヴィアンゲイズ・マグナも実測Lv1・100・150・200を登録しています。Lv100とLv200の単純な中間値は
実測Lv150と一致しないため、Lv100〜150とLv150〜200を別区間として扱います。
テュロスビネットは実測Lv1・100・150を登録し、最大Lv150という条件だけでは禁栄の禍槍と同じ
境界生成係数を使えないことを確認しています。共通プロファイルの識別条件が判明するまでは実測境界を使います。
ビジュアル編集画面では武器ごとに上限解放段階を選び、その段階の上限内で任意の整数Lvを入力できます。
0〜3凸のLv上限はレアリティ共通規則、4凸以降はWiki・実機由来の境界Lvを利用します。既存JSONで
上限解放段階とLvが矛盾する場合はステータスを計算せず、`weapon-level-exceeds-uncap`を返します。
実際のゲーム内表示で確認した
...
`src/calculator/articleBaseDamageCalculator.ts` は、2026年7月30日の有志検証記事
<https://gbf-dmg-calc.hatenablog.com/entry/2026/07/30/184000>をもとにした既定モデルです。
`calculationModel`を省略すると`article-2026-07`を使用します。以前の防御先行モデルは比較用として
`calculationModel: "defense-first-provisional"`で呼び出せます。初期の実験名
`article-2026-07-experimental`も互換用の別名として同じ記事モデルへ接続します。記事モデルでは
表示攻撃力を10分の1単位で
切り上げ、船はfloat64の積を切り上げ、炉は整数%を先に乗じてから割ることで誤差を発生させずに
切り上げます。その後、通常攻刃枠・属性枠・敵防御を適用し、与ダメージ、ジョブ通常攻撃与ダメージ、
対属性与ダメージを同じ加算枠として合算して最後に切り上げます。途中値は`articleTrace`へ保持します。

このモデルは、サポート・アグニス編成の武器+0～+10について、通常・対有利の本家表示22値をすべて
再現します。乱数前の非整数値へ0.001刻みの乱数を掛けて切り上げると通常攻撃本体8件をすべて再現し、
表示値11,300から作る追撃基礎値1,525.5へ独立乱数を掛けて切り捨てる既存モデルでも追撃18件を
すべて再現します。ダメージ上限、分割ヒット、被ダメUPなど記事の後段処理は未実装なので、現時点では
低ダメージ帯の既定モデルとして使用します。
...
| `list_calculator_summons` | 選択式エディタと計算機が解決できる召喚石・加護・サポート選択可否を取得 |
