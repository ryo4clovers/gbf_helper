...
<https://gbf-dmg-calc.hatenablog.com/entry/2026/07/30/184000>を別経路で検証するための
実験モデルです。リクエストに`calculationModel: "article-2026-07-experimental"`を指定した場合だけ
使用し、既定の`defense-first-provisional`は変更しません。記事モデルでは表示攻撃力を10分の1単位で
...
この経路は、サポート・アグニス編成の武器+0～+10について、通常・対有利の本家表示22値をすべて
