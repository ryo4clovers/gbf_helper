...
- メイン1枠と通常9枠の選択式エディタで、登録済み武器の検索・配置・削除・Lv・SLv変更を行う
- 検証済みのレベル境界値がある武器は、Lv変更時に区間式でHP・ATKを自動再計算する
...
ビジュアル編集画面の武器Lvは、カタログに実測または図鑑由来で登録した境界Lvだけを選択できます。
既存JSONに中間Lvがある場合はデータを勝手に変更せず、未検証の無効な選択肢として表示します。
実際のゲーム内表示で確認した
