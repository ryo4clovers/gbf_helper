...
test("weapon slots link uncap to integer equipment and skill level controls", async () => {
  const app = await readFile(new URL("../web/app.js", import.meta.url), "utf8");
  ...
  assert.match(app, /maximumSkillLevelForUncap\(value, master\?\.skillLevelCap\?\.maximum\)/u);
  assert.match(app, /skillInput\.max = String\(maximumSkillLevel\)/u);
  assert.match(app, /skillInput\.disabled = maximumSkillLevel === 1/u);
