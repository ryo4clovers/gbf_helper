      ...
      weapons: [
        {
          ...
          weaponKindCode: "10",
          rarityCode: "4",
          selectionDefaults: { level: 250, uncapLevel: 6, skillLevel: 25, hp: 400, attack: 4000 },
          levelStats: {
            maximumLevel: 250,
            points: [
    ...
    },
    {
      wikiCatalog: {
        weapons: [{
          ...
          uncaps: { maximum: 6 },
          skills: [],
          chargeAttacks: [{ stage: 3, name: "最終奥義", description: "特大ダメージ" }],
        }],
      },
...
test("同期は閲覧用2タブだけを書き換え、余った旧行を後から消去する", async (context) => {
  ...
  assert.deepEqual(
    ...
    ["'武器カタログ'!A1:AJ2", "'スキル・効果'!A1:K3"],
  );
  assert.ok(updateBody.data.every((entry) => !entry.range.includes("提案")));

  const clearBody = JSON.parse(calls[2].init.body);
  assert.deepEqual(clearBody.ranges, ["'武器カタログ'!A3:AJ", "'スキル・効果'!A4:K"]);
  assert.deepEqual(result, {
    weaponRows: 1,
    skillRows: 2,
    clearedRanges: ["'武器カタログ'!A3:AJ", "'スキル・効果'!A4:K"],
  });
});
...
test("同期先の行列が不足している場合は値を書き込む前に拡張する", async (context) => {
  ...
  assert.deepEqual(expansionBody.requests, [
    ...
    { appendDimension: { sheetId: 11, dimension: "COLUMNS", length: 28 } },
