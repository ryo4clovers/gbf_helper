...
| 段階 | Lv | HP | ATK | 備考 |
| --- | --- | --- | --- | --- |
| 初期 | 1 | 40 | 325 | ゲーム内図鑑詳細の`default_hp` / `default_attack` |
| 3凸・最大 | 100 | 258 | 1960 | 実機実測(+0、evolution 3) |
| 5凸後・境界値 | 150 | 302 | 2290 | 実機実測(+0) |
| 5凸・検証点 | 151 | 302 | 2293 | 実機実測(+0) |
| 5凸・検証点 | 152 | 302 | 2296 | 実機実測(+0) |
...
- Lv40・60・80の各上限解放境界値。ただし同一区間式上では追加境界を必要としない。
- Lv150〜200区間式はLv151・152で一致。他のマグナ武器でも同じ規則か。
- Lv1・最終値からLv100/150を生成する共通係数候補（Lv100は約76.95%、Lv150は92.5%）が他の最大Lv200武器にも適用できるか。
