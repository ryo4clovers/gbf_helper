---
id: "dark-ssr-dark-opus-katana-normal"
name_jp: "絶対否定の太刀"
name_en: "Dark Opus Katana (Normal ATK)"
weapon_id: "1040911000"
element: 闇
rarity: SSR
weapon_type: "刀"
series: "終末の神器(通常攻刃)"
...
# 絶対否定の太刀(Dark Opus Katana (Normal ATK))
![絶対否定の太刀](https://prd-game-a-granbluefantasy.akamaized.net/assets/img_low/sp/assets/weapon/m/1040911000.jpg)

## 概要

闇属性の終末の神器(gbf.wiki『Dark Opus Weapons』)。第1スキルの攻刃が**通常攻刃系**(神石グリッド向け)。同じ刀の方陣版は [dark-ssr-dark-opus-katana-magna.md](dark-ssr-dark-opus-katana-magna.md)。第2スキル(α/β/γ/Δ リベレイション)・第3スキル(渾身/背水/技巧/連撃/神闇の極技・極破 等)はゴーフ・キーで**選択式**で、レスポンスには現在の選択のみ出る。超越Lv250。
...
| --- | --- | --- | --- | --- |
| 3凸・最大 | 150 | 要検証 | 要検証 | |
| 超越・最大 | 250 | 408 | 4985 | 実機実測(+0、evolution 6 / phase 5) |
...
- skill_id: `2229`
- 解放レベル: 武器Lv1
- 選択式: はい(ゴーフ・キー)。この個体は「神闇の極技」
- 系統/カテゴリ: 極技/極破(攻撃力 + 各種ダメージ上限 + 与ダメージ の複合、発動条件付き)。image `skill_job_weapon`
- 枠(frame): 要検証
- 効果(実機 comment): 闇属性キャラの攻撃力上昇/ダメージ上限上昇/アビリティダメージ上限上昇/アビリティの与ダメージUP / ◆スキル「黒霧方陣」または「冥堕禁呪」または「闇」「憎悪」「奈落」の効果上昇量が合計280%以上で発動
- スキルレベル別の数値: 要検証
...
- `param.image_id` = `1040911000_03`(超越後の接尾辞)
...
- 使用グリッド: 闇神石グリッドの主力枠。神醒(攻刃+HP+与ダメ)+ 選択スキルで火力と上限を両立。
