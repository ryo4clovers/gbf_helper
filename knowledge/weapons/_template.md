...
- 系統/カテゴリ: (例: 攻刃〈通常攻刃〉/ 方陣攻刃〈オメガ〉/ 神威〈EX〉/ 背水 / 渾身 / 守護 / 治癒 / 特殊 …)
- 枠(frame): (ダメージ計算上の乗算枠。通常 / 別枠 / EX / オメガ / 神石 / アビ与ダメ 等。要検証で可)
- 効果: (実機 comment のまま + 補足)
- スキルレベル別の数値: (Lv1 / Lv10 / Lv15 / Lv20 等。gbf.wiki 出典。未取得なら「要検証」)
- 発動条件: (HP依存 / 得意武器数依存 / 常時 等、あれば)
- 出典:
- ステータス: 未検証
...
<!-- skill3 / skill4 があるジョブ専用武器・上限解放で解放されるスキルは同じ形式で追加。
     実機レスポンスの skillN_display フラグ(0/1)で表示スロットが分かる。 -->

## 奥義(チャージアタック)

- 名称: (実機レスポンス special_skill.name)
- 効果: (special_skill.comment。属性ダメージ倍率〈小/中/大/特大〉+ 追加効果)
- 数値/スケーリング: (奥義倍率・追加バフの%とターン。gbf.wiki 出典。未取得なら「要検証」)
- 上限解放/レベルで変化する場合は段階ごとに記載
- 出典:
- ステータス: 未検証
...
- スキルレベル: (上げ方 / 上限)
- 覚醒(あれば): (実機 param.arousal。覚醒タイプ〈攻撃/防御/連撃/回復/奥義/etc.〉と Lv別ボーナス)
- 超越(あれば): (実機 can_release_transcendence / transcendence。Lv100→200 の強化内容・段階)
