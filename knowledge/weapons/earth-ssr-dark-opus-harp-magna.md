---
id: "earth-ssr-dark-opus-harp-magna"
name_jp: "永遠拒絶の竪琴"
name_en: "Dark Opus Harp (Magna)"
weapon_id: "1040809500"
element: 土
rarity: SSR
weapon_type: "楽器"
series: "終末の神器(方陣攻刃)"
...
# 永遠拒絶の竪琴(Dark Opus Harp (Magna))
![永遠拒絶の竪琴](https://prd-game-a-granbluefantasy.akamaized.net/assets/img_low/sp/assets/weapon/m/1040809500.jpg)

## 概要

土属性の終末の神器(gbf.wiki『Dark Opus Weapons』)。第1スキルの攻刃が**方陣攻刃系**(マグナグリッド向け、image に `_m_`)。同じ楽器の通常版は [earth-ssr-dark-opus-harp-normal.md](earth-ssr-dark-opus-harp-normal.md)。第2スキル(α/β/γ/Δ リベレイション)・第3スキル(渾身/背水/技巧/連撃/神土の極技・極破 等)はゴーフ・キーで**選択式**で、レスポンスには現在の選択のみ出る。超越Lv250。
...
| --- | --- | --- | --- | --- |
| 3凸・最大 | 150 | 要検証 | 要検証 | |
| 超越・最大 | 250 | 385 | 3690 | 実機実測(+0、evolution 6 / phase 5) |
...
- 解放レベル: 武器Lv230
- 選択式: いいえ(固定)
- 系統/カテゴリ: 神醒(神醒III)。方陣攻刃 + 最大HP + 与ダメージUP の複合(image `skill_atk_hp_rise_m_3_3` の `_m_`)
- 枠(frame): 方陣攻刃枠 + HP + 与ダメージ上昇枠(複合、要検証)
- 効果(実機 comment): 土属性キャラの攻撃力と最大HPが上昇(大)/与ダメージUP(大)
- スキルレベル別の数値: 神醒III(大)スキルLv25 時の各%は**要検証**
- 発動条件: 常時(土属性キャラ)。武器Lv230(超越)で解放
- image: `skill_atk_hp_rise_m_3_3`
- 出典: 実機レスポンス
- ステータス: 未検証
...
- 名称: アポカリプス・アーク・ドライヴ＋
- 効果(実機 comment): 土属性ダメージ(極大)/味方全体に荒土の刻印を付与/攻防UP(累積)/スキルに応じた追加効果
...
- `param.image_id` = `1040809500_03`(超越後の接尾辞)
...
- 使用グリッド: 土マグナグリッドの主力枠。神醒(攻刃+HP+与ダメ)+ 選択スキルで火力と上限を両立。
- メイン装備時の恩恵: 奥義「アポカリプス・アーク・ドライヴ＋」(刻印付与 + 攻防UP累積)。
...
- ゴーフ・キーの全選択肢一覧と各効果量。
- 途中の上限解放/超越段階のステータス。
- 奥義「アポカリプス・アーク・ドライヴ＋」の倍率、刻印効果、攻防UP(累積)の上限。
- 各スキルの正確な枠(frame)。
