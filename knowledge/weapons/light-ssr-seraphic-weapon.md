---
id: "light-ssr-seraphic-weapon"
name_jp: "永遠の知識求る竪琴"
name_en: "Harp of the Teachers"
weapon_id: "1040809600"
element: 光
rarity: SSR
weapon_type: "楽器"
series: "セラフィックウェポン"
...
# 永遠の知識求る竪琴(Harp of the Teachers)
![永遠の知識求る竪琴](https://prd-game-a-granbluefantasy.akamaized.net/assets/img_low/sp/assets/weapon/m/1040809600.jpg)

## 概要

光属性のセラフィックウェポン。編成に1本のみ装備でき、有利属性(光→闇)の敵に対する光パーティの与ダメージを底上げする特殊枠。グリッド火力より「与ダメージ上昇」と奥義の光属性攻撃UP/不利属性ダメージ軽減が目的で、有利古戦場・高難易度で1枠採用される。火版は [fire-ssr-seraphic-weapon.md](fire-ssr-seraphic-weapon.md)。
...
| --- | --- | --- | --- | --- |
| 初期 | 1 | 要検証 | 要検証 | |
| 最終上限解放・最大 | 150 | 270 | 2660 | 実機実測(`param.hp`/`param.attack`、+0、evolution 4) |
...
- 解放レベル: 武器Lv1
- 選択式: いいえ(固定)
- 系統/カテゴリ: セラフィック(有利属性の敵に対する与ダメージ上昇)。image `skill_seraphic_5_3`
- 枠(frame): 与ダメージ上昇枠(他の与ダメージUP系と加算される独立枠、要検証)
- 効果(実機 comment): 光属性のキャラが闇属性の敵に対して与ダメージUP
- スキルレベル別の数値: 刷新後・スキルLv15 時の与ダメージUP% は**要検証**(gbf.wiki は刷新前『祝福II』基準)
- 発動条件: 光属性キャラ かつ 敵が闇属性のとき
- image: `skill_seraphic_5_3`
- 出典: 実機レスポンス
- ステータス: 未検証
...
- 解放レベル: 武器Lv1
- 選択式: いいえ(固定)
- 系統/カテゴリ: 神威(EX攻刃 + 最大HP UP、小)。image `skill_god_5`
...
- 名称: エターナルグレイス
- 効果(実機 comment): 光属性ダメージ(特大)/味方全体の光属性攻撃UP/闇属性ダメージ軽減
- 数値/スケーリング: 奥義倍率、光属性攻撃UP・闇属性ダメージ軽減の % とターン数は**要検証**
- 出典: 実機レスポンス
- ステータス: 未検証
