---
id: "water-ssr-dark-opus-cane-normal"
name_jp: "絶対否定の杖"
name_en: "Dark Opus Cane (Normal ATK)"
weapon_id: "1040415000"
element: 水
rarity: SSR
weapon_type: "杖"
series: "終末の神器(通常攻刃)"
...
# 絶対否定の杖(Dark Opus Cane (Normal ATK))
![絶対否定の杖](https://prd-game-a-granbluefantasy.akamaized.net/assets/img_low/sp/assets/weapon/m/1040415000.jpg)

## 概要

水属性の終末の神器(gbf.wiki『Dark Opus Weapons』)。第1スキルの攻刃が**通常攻刃系**(神石グリッド向け)。同じ杖の方陣版は [water-ssr-dark-opus-cane-magna.md](water-ssr-dark-opus-cane-magna.md)。第2スキル(α/β/γ/Δ リベレイション)・第3スキル(渾身/背水/技巧/連撃/神水の極技・極破 等)はゴーフ・キーで**選択式**で、レスポンスには現在の選択のみ出る。超越Lv250。
...
| --- | --- | --- | --- | --- |
| 3凸・最大 | 150 | 要検証 | 要検証 | |
| 超越・最大 | 250 | 581 | 4100 | 実機実測(+0、evolution 6 / phase 5) |
...
- skill_id: `2213`
- 解放レベル: 武器Lv1
- 選択式: はい(ゴーフ・キー)。この個体は「神水の極破」
- 系統/カテゴリ: 極技/極破(攻撃力 + 各種ダメージ上限 + 与ダメージ の複合、発動条件付き)。image `skill_job_weapon`
- 枠(frame): 要検証
- 効果(実機 comment): 水属性キャラの攻撃力上昇/ダメージ上限上昇/連続攻撃確率上昇/通常攻撃の与ダメージUP / ◆スキル「海神方陣」または「怪水禁呪」または「水」「渦潮」「霧氷」の効果上昇量が合計280%以上で発動
- スキルレベル別の数値: 要検証
...
- `param.image_id` = `1040415000_03`(超越後の接尾辞)
...
- 使用グリッド: 水神石グリッドの主力枠。神醒(攻刃+HP+与ダメ)+ 選択スキルで火力と上限を両立。
