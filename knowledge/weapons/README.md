...
グランブルーファンタジーの武器を、検証作業用Markdownと公開Wiki由来の構造化カタログでまとめるカテゴリ。
将来のダメージ計算機・グリッド編成ツールの素材とするため、**スキルの系統・枠・スキルレベル別の数値**を重視する。

## 収集方針

- 実機検証用MarkdownはSSRを中心とし、ユーザーが Network Recorder で武器詳細レスポンスを収集し `draft/武器/{シリーズ名}/` に配置 → シリーズ単位で一括処理する。2026-09-09から、公開Wikiだけで取得できるN〜SSRの基本情報は構造化カタログにも収録する。
...
| --- | --- |
| **レアリティ `rarity`** | 1 N / 2 R / 3 SR / 4 SSR |
| **武器種 `kind`** | 1 剣(確認済) / 2 短剣 / 3 槍(確認済) / 4 斧(確認済) / 5 杖(確認済) / 6 銃(確認済) / 7 格闘 / 8 弓 / 9 楽器(確認済) / 10 刀(未確認分はジョブの `weapon1/2` コードと同じと推定) |
| **シリーズ `series_id`** | 1 セラフィック / 2 リミテッド / 3 終末の神器(Dark Opus)/ 5 プライマル / 7 レガリア(マグナII)/ 8 マグナ(旧マグナ。Omega)/ 13 オメガウェポン(Ultima)/ 14 バハムート / 19 英雄武器(ジョブ専用。Class Champion)/ 26 アストラル / 29 アンセスタル(六竜武器)/ 30 新世界の礎(賢者武器。NWF)/ 31 エニアド / 33 マリス / 34 メナス / 37 レヴァンス / 40 ドラゴニック・オリジン / 42 マグナ・リバース(マグナIII)/ 44 破壊の標(ヴェルサシア)/ 45 禁禍武器(Verboten)。連番ではないので実例で確認する |
...
- **Exスキル**: 下記「Exスキル」節を参照。個体差のため各武器ファイルには記載しない。
- **AUG(エレメント)**: プレイヤーが素材で付与する別枠の追加効果(`augment_skill`)。Exスキルとは別。旧マグナ等では個体固有情報のため各武器ファイルには記載しない(禁禍武器のデメリットスキルのみ例外的に記載)。
...
## Exスキル(ドロップ時ランダム付与スキル)

一部シリーズの武器は、**ドロップした時点でランダムな追加スキル(Exスキル)が付与されている**ことがある。個体ごとに内容が異なるため、**各武器の Markdown には記載しない**(武器固有スキルのみ記載する)。ここに付与シリーズと抽選パターンをまとめる。

### 付与されるシリーズ

| シリーズ | `series_id` | 備考 |
| --- | --- | --- |
| マグナシリーズ(旧マグナ / マグナ1) | 8 | 付与率 約60%(サンプル調査値)。レプリカルド・サンドボックスや旧マグナHLでExスキル付きがドロップ |
| アンセスタルシリーズ(六竜武器) | 未確認 | Exスキル2に六竜武器専用の候補(属性ダメージ軽減・アビ/奥義与ダメージ)が追加される |
| 六道武器 / ゼノ武器 | 未確認 | 神ゲー攻略・ゲーム乱舞が付与対象として記載 |
| プライマル / オールドプライマル | 未確認 | ユーザー談。二次サイトでは明示されず要確認 |

※ レガリアシリーズ(`series_id` 7、マグナII)・マグナ・リバースシリーズ(`series_id` 42、マグナIII)の第2スキル以降は**武器固有スキル**であり Exスキルではない(個体差なし)。

### 仕様(GameWith / 神ゲー攻略 / ゲーム乱舞)

- **付与数**: 1本につき Exスキルは **1〜2個**。付く場合、第1スキルが「Exスキル1」、第2スキルが「Exスキル2」の位置(旧マグナのように武器固有スキルが第1にある場合は第2スキル枠のみ)。
- **抽選**: ドロップ時に固定。武器固有スキルは変わらず、Exスキル枠のみランダム。
- **効果量**: **スキルレベルを上げても効果量は上昇しない**(入手時の値で固定)。
- **レベル継承**: 武器強化画面の「レベル継承」で、**同名武器から Lv / スキルLv / 上限解放状況を継承できる**(継承元の Exスキルは移らない)。= 良い Exスキルを引いた個体を、他の同名武器を食わせて安く育てられる。装備中の武器も継承可。保護中の武器は素材一覧に出ないので保護解除が必要。

### Exスキルの効果一覧(効果量レンジ)

**Exスキル1(第1スキル位置。旧マグナでは第2スキル枠にこの群が出ることもある):**

| 効果 | 効果量 | 枠(frame) |
| --- | --- | --- |
| HP | 1〜11% | HP |
| 攻撃力 | 1〜3.5% | **乗算枠。久遠の指輪と同枠で加算**(= EX/久遠枠) |
| 防御力 | 1〜8% | 防御 |
| 連続攻撃確率(DA/TA) | 1.5〜4% | 連撃 |
| 奥義ダメージ | 2〜8.5% | 奥義ダメージ |
...
| --- | --- | --- |
| 属性攻撃力 | 2〜5% | 要検証(EX系) |
| 渾身 | +1〜3 | 渾身(LB/ストレングス等と別、要検証) |
| 背水 | +1〜3% | 背水(LB/逆境等と別、要検証) |
| ダブルアタック / トリプルアタック(個別) | 1〜2% | 連撃 |
| 奥義ダメージ上限 / アビリティダメージ上限 | 1〜2% | 各ダメージ上限 |
| 回復性能 | 2〜5% | 回復 |
| 弱体耐性 | 1〜3% | 弱体耐性 |
...
| --- | --- |
| 属性ダメージ軽減 | 1〜5% |
| アビリティ与ダメージ上昇 / 奥義与ダメージ上昇 | +1〜5 |

### 補足

- 第1スキル(武器固有スキル)の系統ごとに Exスキルの抽選候補が絞られる可能性(tkグラン サンプル調査 n=120)。当リポジトリの旧マグナ19本でも第2スキル枠に「方陣背水」「他属性の通常攻刃/守護/背水」「方陣見切/技巧」等が観測された。
- 攻撃力Exが「久遠枠加算」である点はダメージ計算上重要(久遠の指輪〈エンジェルウェポン〉と同じ枠に足し込まれる)。

出典: [GameWith EXスキル記事](https://xn--bck3aza1a2if6kra4ee0hf.gamewith.jp/article/show/245175) / [神ゲー攻略 EXスキル](https://kamigame.jp/%E3%82%B0%E3%83%A9%E3%83%96%E3%83%AB/%E3%82%A2%E3%83%BC%E3%82%AB%E3%83%AB%E3%83%A0%E3%81%AE%E8%BB%A2%E4%B8%96/EX%E3%82%B9%E3%82%AD%E3%83%AB.html) / [ゲーム乱舞 EXスキルの仕様](https://gameranbu.jp/granblue/dcae66ba3e5ce337dbcb) / [tkグランのグラブル攻略日記(サンプル調査)](https://tkgranblue.com/archives/13539)(いずれも 2026-09-08 参照)
