---
id: "wind-ssr-seraphic-weapon"
name_jp: "果てぬ風呼の魔弓"
name_en: "Ring of Raphael"
weapon_id: "1040709100"
element: 風
rarity: SSR
weapon_type: "弓"
series: "セラフィックウェポン"
...
# 果てぬ風呼の魔弓(Ring of Raphael)
![果てぬ風呼の魔弓](https://prd-game-a-granbluefantasy.akamaized.net/assets/img_low/sp/assets/weapon/m/1040709100.jpg)

## 概要

風属性のセラフィックウェポン。編成に1本のみ装備でき、有利属性(風→土)の敵に対する風パーティの与ダメージを底上げする特殊枠。グリッド火力より「与ダメージ上昇」と奥義の風属性攻撃UP/不利属性ダメージ軽減が目的で、有利古戦場・高難易度で1枠採用される。火版は [fire-ssr-seraphic-weapon.md](fire-ssr-seraphic-weapon.md)。
...
| --- | --- | --- | --- | --- |
| 初期 | 1 | 要検証 | 要検証 | |
| 最終上限解放・最大 | 150 | 270 | 2970 | 実機実測(`param.hp`/`param.attack`、+0、evolution 4) |
...
- 解放レベル: 武器Lv1
- 選択式: いいえ(固定)
- 系統/カテゴリ: セラフィック(有利属性の敵に対する与ダメージ上昇)。image `skill_seraphic_4_3`
- 枠(frame): 与ダメージ上昇枠(他の与ダメージUP系と加算される独立枠、要検証)
- 効果(実機 comment): 風属性のキャラが土属性の敵に対して与ダメージUP
- スキルレベル別の数値: 刷新後・スキルLv15 時の与ダメージUP% は**要検証**(gbf.wiki は刷新前『祝福II』基準)
- 発動条件: 風属性キャラ かつ 敵が土属性のとき
- image: `skill_seraphic_4_3`
- 出典: 実機レスポンス
- ステータス: 未検証
...
- 解放レベル: 武器Lv1
- 選択式: いいえ(固定)
- 系統/カテゴリ: 神威(EX攻刃 + 最大HP UP、小)。image `skill_god_4`
- 枠(frame): EX攻刃枠(攻撃力) + HP。通常攻刃・方陣攻刃とは別枠(要検証)
- 効果(実機 comment): 風属性キャラの攻撃力と最大HPが上昇(小)
...
- 名称: ジャーニーズ・リンク
- 効果(実機 comment): 風属性ダメージ(特大)/味方全体の風属性攻撃UP/土属性ダメージ軽減
- 数値/スケーリング: 奥義倍率、風属性攻撃UP・土属性ダメージ軽減の % とターン数は**要検証**
- 出典: 実機レスポンス
- ステータス: 未検証
