---
id: "earth-ssr-tsuchinoko-normal"
name_jp: "ツチノコ"
name_en: "Tsuchinoko"
rarity: SSR
element: "土"
obtain: "ガチャ排出/サプチケ不可"
status: 下書き
last_updated: 2026-08-20
source: "GameWith SSR召喚石一覧 (https://xn--bck3aza1a2if6kra4ee0hf.gamewith.jp/article/show/136372) と gbf.wiki (https://gbf.wiki/Tsuchinoko) の両方で確認(取得日: 2026-08-20)。ATK/HPともに一致。"
---

# ツチノコ(Tsuchinoko)

## 概要

土属性のSSR召喚石。

## 基本情報

| 項目 | 内容 |
| --- | --- |
| レアリティ | SSR |
| 属性 | 土 |
| 入手方法 | ガチャ排出/サプチケ不可 |

## ステータス

| レベル | HP | ATK |
| --- | --- | --- |
| Lv100(3★) | 875 | 2080 |
| Lv150(4★・最終上限解放) | 1236 | 2965 |

## 召喚効果

### 初期

- 効果: 
- 初回召喚: 12ターン後
- 再召喚: 可
- 出典: GameWith(個別ページ)

### 初期

- 効果: 
- 初回召喚: 12ターン後
- 再召喚: 可
- 出典: GameWith(個別ページ)

### 4凸

- 効果: 敵全体のアイテムドロップ率UP(?回) / 味方全体が即座に奥義発動可能 / ◆再召喚不可/参戦者が合体召喚不可
- 初回召喚: 12ターン後
- 再召喚: 不可
- 出典: GameWith(個別ページ)

## 加護効果(メイン編成時)

- 効果: 土属性攻撃力が100%UP/バトル開始時にパーティの奥義ゲージ30%UP / (3凸)土属性攻撃力が120%UP/バトル開始時にパーティの奥義ゲージ30%UP / (4凸)土属性攻撃力が130%UP/バトル開始時にパーティの奥義ゲージ30%UP/バトル終了時にツチノコに遭遇することがある
- 出典: GameWith

## サブ加護効果(サブ編成時)

- 効果: (4凸)バトル終了時にツチノコに遭遇することがある
- 出典: GameWith

## 関連トピック

- [buffs-debuffs.md](../mechanics/buffs-debuffs.md)
- [damage-cap-modifiers.md](../mechanics/damage-cap-modifiers.md)

## 未確認・要検証事項

- サブ加護効果の正確な倍率はGameWith一覧に明記されていない場合、未検証。
- gbf.wikiでさらに上位の上限解放段階(5★/6★等)が存在する場合、本ファイルはGameWithの標準的な表記(3★/4★)までの記載としており、それ以降は未反映。
