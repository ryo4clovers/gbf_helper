---
id: "earth-ssr-yggdrasil-summer"
name_jp: "水着ユグドラシル"
name_en: "Yggdrasil (Summer)"
rarity: SSR
element: "土"
obtain: "ガチャ排出/サプチケ不可"
status: 下書き
last_updated: 2026-08-20
source: "GameWith SSR召喚石一覧 (https://xn--bck3aza1a2if6kra4ee0hf.gamewith.jp/article/show/136372) と gbf.wiki (https://gbf.wiki/Yggdrasil_(Summer)) の両方で確認(取得日: 2026-08-20)。ATK/HPの一部に不一致あり(未確認・要検証事項を参照)。"
---

# 水着ユグドラシル(Yggdrasil (Summer))

## 概要

土属性のSSR召喚石。

## 基本情報

| 項目 | 内容 |
| --- | --- |
| レアリティ | SSR |
| 属性 | 土 |
| 入手方法 | ガチャ排出/サプチケ不可 |

## ステータス

| レベル | HP | ATK |
| --- | --- | --- |
| Lv100(3★) | 755 | 2002 |
| Lv150(4★・最終上限解放) | 1030 | 2500 |

## 召喚効果

- 効果: 土属性キャラにバリア効果/活性効果 / (3凸)土属性キャラにバリア効果/活性効果 / (4凸)土属性キャラにバリア効果/活性効果/弱体効果無効(1回)
- 出典: GameWith

## 加護効果(メイン編成時)

- 効果: 土属性攻撃力が80%UP / (4凸)土属性攻撃力が110%UP
- 出典: GameWith

## サブ加護効果(サブ編成時)

- 加護効果と同一、または倍率のみ異なる想定(GameWith一覧には明記なし、要検証)。

## 関連トピック

- [buffs-debuffs.md](../mechanics/buffs-debuffs.md)
- [damage-cap-modifiers.md](../mechanics/damage-cap-modifiers.md)

## 未確認・要検証事項

- サブ加護効果の正確な倍率はGameWith一覧に明記されていない場合、未検証。
- gbf.wikiとの間でステータスに不一致あり(GameWith: HP755/ATK2002 vs gbf.wiki Lv100: {"hp":"820","atk":"2002"})。要ゲーム内再検証。
- gbf.wikiでさらに上位の上限解放段階(5★/6★等)が存在する場合、本ファイルはGameWithの標準的な表記(3★/4★)までの記載としており、それ以降は未反映。
