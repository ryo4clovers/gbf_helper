---
id: "fire-ssr-satyr-summer"
name_jp: "水着サテュロス"
name_en: "Satyr (Summer)"
rarity: SSR
element: "火"
obtain: ""
status: 下書き
last_updated: 2026-08-20
source: "GameWith SSR召喚石一覧 (https://xn--bck3aza1a2if6kra4ee0hf.gamewith.jp/article/show/136372) と gbf.wiki (https://gbf.wiki/Satyr_(Summer)) の両方で確認(取得日: 2026-08-20)。ATK/HPともに一致。"
---

# 水着サテュロス(Satyr (Summer))

## 概要

火属性のSSR召喚石。

## 基本情報

| 項目 | 内容 |
| --- | --- |
| レアリティ | SSR |
| 属性 | 火 |
| 入手方法 |  |

## ステータス

| レベル | HP | ATK |
| --- | --- | --- |
| Lv100(3★) | 810 | 2050 |
| Lv150(4★・最終上限解放) | 979 | 2476 |

## 召喚効果

- 効果: 3ターンの間、自分に星トモの夏効果(消去不可) / 火属性キャラに風属性ダメージ20%カット / ◆星トモの夏：敵が特殊技を使用した時、味方全体のHPを20%回復(最大2500)/弱体効果を1つ回復 / (3凸)3ターンの間、自分に星トモの夏効果(消去不可) / 火属性キャラに風属性ダメージ40%カット / ◆星トモの夏：敵が特殊技を使用した時、味方全体のHPを回復/弱体効果を1つ回復 / (4凸)3ターンの間、自分に星トモの夏効果(消去不可) / 風属性ダメージカットが50%に強化
- 出典: GameWith

## 加護効果(メイン編成時)

- 効果: 火属性攻撃力が80%UP / (3凸)火属性攻撃力が100%UP / (4凸)火属性攻撃力が110%UP
- 出典: GameWith

## サブ加護効果(サブ編成時)

- 加護効果と同一、または倍率のみ異なる想定(GameWith一覧には明記なし、要検証)。

## 関連トピック

- [buffs-debuffs.md](../mechanics/buffs-debuffs.md)
- [damage-cap-modifiers.md](../mechanics/damage-cap-modifiers.md)

## 未確認・要検証事項

- サブ加護効果の正確な倍率はGameWith一覧に明記されていない場合、未検証。
- gbf.wikiでさらに上位の上限解放段階(5★/6★等)が存在する場合、本ファイルはGameWithの標準的な表記(3★/4★)までの記載としており、それ以降は未反映。
